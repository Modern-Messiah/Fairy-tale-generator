...
class StoryRequest(BaseModel):
    ...
    age: int = Field(
        ..., gt=0, le=10, description="Возраст ребенка (от 1 до 10 лет)", examples=[6]
    )
    ...
    genre: str = Field(..., description="Жанр сказки", examples=["adventure"])
    ...
    def validate_genre(cls, v) -> str:
        ...
        if v not in allowed:
            raise ValueError(f"Жанр должен быть одним из: {','.join(allowed)}")
        return v
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [
                ...
                {"age": 7, "language": "ru", "characters": ["Заяц", "Волк", "Медведь"]},
            ]
        }
    }
